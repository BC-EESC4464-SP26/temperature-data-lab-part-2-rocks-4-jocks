...
% repository
addpath("model_data");
addpath("C:\Users\nuhin\git\temperature-data-lab-part-1-rocks-4-jocks\ObsData");
...
for i = 1:length(sta)
    filename = sprintf("%d.csv", sta(i));
    stationData = readtable(filename);
    years = stationData{:,3};
    temp = stationData{:, 4:end};

    meanTemps = mean(temp, 2, 'omitnan');

    recentIndices = years >= RecentYear;
    
    years_recent = years(recentIndices);
    temps_recent = meanTemps(recentIndices);
    
    [slope_recent, intercept_recent] = linear_temp(years_recent, temps_recent);
    P_all(i,:) = [slope_recent, intercept_recent];
...
load coastlines
plotm(coastlat,coastlon)
scatterm(lat, lon, 50, P_all(:, 1), 'filled');
colorbar;
title('Rate of Temperature Change at Each Station');

% Temperature is increasing fastest at Southern Europe, Australia, and
% Alaska. Temperature is increasing slowest at North America, Asia like
% Korea and India, and South America.

% For rates of change at the 18 stations, they match for some of the IPCC
% 4th Assessment Report for areas like India and North America. However,
% for areas like Australia, East and Central Asia, they do not match with the
% Report. 
...
modelArray = NaN(18,2); 
baselineArray = NaN(18,2);

for i = 1:18
    
    [baseline_model, P] = StationModelProjections(sta(i));
    
    baselineArray(i,:) = baseline_model;
    modelArray(i,:) = P;
